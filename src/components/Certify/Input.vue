...
<script>
    ...
    import { checkIdNumber, verifyTime, onlyNumber, onlyStr, formatBankNumber, trim, checkMob, checkPhone, checkSocialCreditCode } from '../../tools/operation';
    ...
    export default {
        ...
        methods: {
            ...
            submit(){
                ...
                if(!checkSocialCreditCode(enterprise_registration_no)){
                    this.messageError('请填写正确格式的企业证件号码！');
                    return false;
                }
                if(!trim(enterprise_registration_no)){
                    this.messageError('请填写企业证件号码！');
            ...
            getData(data){
                ...
                this.showFile('enterprise_card_file');
                this.showFile('corporation_id_face_file');
                this.showFile('corporation_id_back_file');
            },
            showFile(key){
                //返显附件
                this[key] = JSON.parse(this[key]);
                let { unique_file_name, file_name } = this[key];
                $api.get(`/bizeff/merchants/file/${unique_file_name}`).then(res => {
                    if(res.resp_code == 200){
                        this.attachmentItems[key].fileList = [];
                        this.attachmentItems[key].fileList.push({
                            name:file_name,
                            url:res.data.url
                        });
                    }else{
